...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((onResolve, onReject) => {
    if (typeof isPositiveAnswer !== 'boolean') {
      onReject(new Error('Wrong parameter is passed! Ask her again.'));
    } else if (isPositiveAnswer) {
      onResolve('Hooray!!! She said "Yes"!');
    } else {
      onResolve('Oh no, she said "No".');
    }
  });
}
...
function chainPromises(array, action) {
  return array
    .reduce((accum, newPromise) => accum.then(
      (result) => newPromise.then(
        (newResult) => action(result, newResult),
        () => new Error('Error'),
      ),
      () => new Error('Error'),
    ));
